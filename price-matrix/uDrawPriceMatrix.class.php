...
if (!class_exists('uDrawPriceMatrix')) {
    class uDrawPriceMatrix {
        ...
        public function init() {
            global $woocommerce;
            add_action('admin_menu', array(&$this, 'admin_add_menu_pages'), 20);
        ...
        public function init_db() {
            ...
            // Create hidden product to handle price matrix products.            
            $current_hidden_product_id = $this->get_price_matrix_product_id(); 
            
            if ( FALSE === get_post_status( $current_hidden_product_id ) ) {
                // product id was removed or not found. We will create it again in this casse.
                $current_hidden_product_id = 0;
            }        
            
            if ($current_hidden_product_id == 0) {
                $hidden_product = array(
                    'post_author' => wp_get_current_user()->ID,
                    'post_content' => '',
                    'post_status' => "publish",
                    'post_title' => '_udraw_price_matrix_product',
                    'post_name' => '_udraw_price_matrix_product',
                    'post_parent' => '',
                    'post_type' => "product",
                    'post_excerpt' => '_udraw_price_matrix_product'
                );

                $hidden_product_id = wp_insert_post($hidden_product);
                if ($hidden_product_id > 0) {
                    wp_update_post(array(
                        'ID' => $post_id,
                        'comment_status' => 'closed'
                    ));
                    update_post_meta($hidden_product_id, '_visibility', 'hidden');
                    update_post_meta($hidden_product_id, '_stock_status', 'instock');
                    update_post_meta($hidden_product_id, 'total_sales', '0');
                    update_post_meta($hidden_product_id, '_downloadable', 'no');
                    update_post_meta($hidden_product_id, '_virtual', 'no');
                    update_post_meta($hidden_product_id, '_regular_price', "100.00");
                    update_post_meta($hidden_product_id, '_sale_price', "");
                    update_post_meta($hidden_product_id, '_purchase_note', "");
                    update_post_meta($hidden_product_id, '_featured', "no");
                    update_post_meta($hidden_product_id, '_weight', "");
                    update_post_meta($hidden_product_id, '_length', "");
                    update_post_meta($hidden_product_id, '_width', "");
                    update_post_meta($hidden_product_id, '_height', "");
                    update_post_meta($hidden_product_id, '_sku', "");
                    update_post_meta($hidden_product_id, '_product_attributes', array());
                    update_post_meta($hidden_product_id, '_sale_price_dates_from', "");
                    update_post_meta($hidden_product_id, '_sale_price_dates_to', "");
                    update_post_meta($hidden_product_id, '_price', "100");
                    update_post_meta($hidden_product_id, '_sold_individually', "no");
                    update_post_meta($hidden_product_id, '_manage_stock', "no");
                    update_post_meta($hidden_product_id, '_backorders', "no");
                    update_post_meta($hidden_product_id, '_stock', "");
                    update_post_meta($hidden_product_id, '_price_matrix_product', true);
                    update_post_meta($hidden_product_id, '_udraw_product', 'true');
                }

                // Update Settings.
                if (is_multisite()) {
                    update_site_option('udraw_price_matrix_product_id', $hidden_product_id);
                } else {
                    update_option('udraw_price_matrix_product_id', $hidden_product_id);
                }
            }
        ...
        public function registerScripts() {
            ...
            wp_register_script('price-matrix-js', plugins_url('includes/price-matrix.js', __FILE__));
            wp_register_script('udraw-price-matrix-js', plugins_url('includes/udraw-price-matrix.js', __FILE__));                        
        ...
        public function get_item_data($other_data, $cart_item) {
            ...
            if (isset($udraw_data['udraw_price_matrix_selected_options']) && isset($udraw_data['udraw_price_matrix_qty']) ) {                                
                //Price matrix selected quantity
                array_push($other_data, array( 'name' => 'Quantity', 'value' => $udraw_data['udraw_price_matrix_qty']));
                if (isset($udraw_data['udraw_price_matrix_selected_options_object'])) {
                    $selected_options = json_decode(stripcslashes($udraw_data['udraw_price_matrix_selected_options_object']));
                    for ($x = 0; $x < count($selected_options); $x++) {
                        if ($selected_options[$x]->name == "_CanvasRatio") { continue; }
        ...
        public function checkout_update_order_meta($order_id) {
            ...
            for ($x = 0; $x < count($item_keys); $x++) {
                ...
                if (strlen($uploaded_files) > 0) {
                    foreach ($uploaded_files as $upload_file) {
                        wc_add_order_item_meta($item_keys[$x], "Attached (" . $upload_file->name . ")" ,
                                               "<a href='". wp_make_link_relative($upload_file->url) ."'>Download</a>", false);
                    }                
                }
            }
            /*
             * Not needed, called in uDraw Class
             * 
             * $udraw = new uDraw();
             * $udraw->generate_pdf_from_order($order_id, false);
             */
        }
                ...
                if (isset($cart_item['udraw_data'])) {
                    if (isset($cart_item['udraw_data']['udraw_price_matrix_selected_options_idx'])) {
                        if (isset($cart_item['data'])) {
                            ...
                            $product = $_pf->get_product($cart_item['data']->id);
                            $cart_item_name = $name;
        ...
        public function order_item_thumbnail( $default, $item ) {
            if ($item["product_id"] == $this->get_price_matrix_product_id()) {
                ...
                if (strlen($udraw_data['udraw_price_matrix_design_preview']) > 1) {
                    $_previewURL = $udraw_data['udraw_price_matrix_design_preview'];
                }
                
                $response = '<img style="width:120px; height:90px;" src="'. $_previewURL .'" class="attachment-shop_thumbnail wp-post-image" alt="uDraw Product">';                
                
                if (isset($udraw_data['udraw_price_matrix_name'])) {
                    $response .= '<br /><label><strong>' . $udraw_data['udraw_price_matrix_name'] . '</strong></label>';
                }
                return $response;
            } else {
                return $default;
        ...
        public function get_product_price_matrix_key($product_id) {
            ...
            if ($terms) {
                foreach ($terms as $term) {
                    ...
                    if (count($price_matrix_object) > 0 && $price_matrix_object !== '') {
        ...
        public function handle_ajax_save() {
            if (isset($_REQUEST['name']) && isset($_REQUEST['xml'])) {
                ...
                isset($_REQUEST['disable_file_upload']) && isset($_REQUEST['disable_design_online']) && isset($_REQUEST['udraw_template_id']) &&
                isset($_REQUEST['measurement_label']) ) {
                    $this->handle_ajax_update_preview_settings($access_key, $_REQUEST['font_color'], $_REQUEST['background_color'],$_REQUEST['disable_file_upload'],$_REQUEST['disable_design_online'],$_REQUEST['udraw_template_id'],$_REQUEST['measurement_label']);
                }
        ...
        public function handle_ajax_upload() {            
            ...
            $validExt = array (
                ...
                'tif' => 'image/tiff', 'ai' => 'application/postscript', 'cdr' => 'application/octet-stream', 'eps' => 'application/postscript', 'ps' => 'application/postscript',
            ...
            $uploaded_files = $uDrawUpload->handle_upload($post_files, $upload_dir, $upload_url, $validExt);
                ...
                if ( !key_exists('error', $uploaded_files[0]) ) {
                    ...
                    if (isset($_REQUEST['greyscale'])) {
                        if (strtolower($fileExt) == 'pdf') {
                            ...
                            $key = base64_encode(uDraw::get_udraw_activation_key() .'%%'. str_replace('.', '-', $_SERVER['HTTP_HOST']));
                            $type = 'pdfbw';
                            $pdf_action = 'convert';
                            $file = $fileObj->url;
                            $uDrawUtil->download_file(UDRAW_CONVERT_URL."key=".$key."&type=".$type."&action=".$pdf_action."&document=".urlencode($file), $output_file);
