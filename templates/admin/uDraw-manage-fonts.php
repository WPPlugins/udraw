...
//File upload stuff
    if (!empty($_FILES['files'])) {
        ...
        $uploaded_files = $uDrawUpload->handle_upload($_FILES['files'], UDRAW_FONTS_DIR, UDRAW_FONTS_URL, array( 'woff' => 'application/font-woff' ));
        ...
        if (is_array($uploaded_files)) {
            for ($x = 0; $x < count($uploaded_files); $x++) {
                if ( !key_exists('error', $uploaded_files[$x]) ) {
                    ...
                    echo '<div class="error" style="padding: 10px; margin-left: 0px;"><span id="upload-error-span"><i class="fa fa-times-circle" style="color: red; transform: scale(1.5,1.5); margin-right: 5px;"></i>An error occurred. Please ensure that the file type is .woff.</span></div>';
                }
            }
        }        
    }
...
<div class="wrap" id="udraw-manage-fonts">
    <div class="row">
        <h3 id="font-upload-message"><?php _e('Upload Font(s)', 'udraw') ?> - <strong style="font-size:1.4em; color:#F00;" ><?php _e('Note: Only .woff fonts are supported.', 'udraw') ?></strong></h3>
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <input type="file" name="files[]" id="files" multiple accept=".woff" style="display: none;">
...
<script>
    ...
    <?php if (current_user_can('delete_udraw_fonts')) { ?>
        function removeUDrawFont(name) {
            jQuery.getJSON(ajaxurl + '?action=udraw_designer_remove_font&font_name=' + name,
                function (data) {
