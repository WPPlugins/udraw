...
class uDraw_Block_Templates_Table extends WP_List_Table {
    ...
    function column_linked_products($item) 
    {
        ...
        if (current_user_can('edit_products')) {
                ...
                $response .= "<a class=\"button button-primary button-small\" href=\"post-new.php?post_type=product&udraw_template_id=". $item['UniqueID'] . "&udraw_action=new-block-product\">Link To Product</a>";
            }
        return $response;
    }
    ...
    function prepare_items()
    {
        ...
        $this->items = $uDrawPDFBlocks->get_company_products();                        
        
        
        for($i = 0; $i < count($this->items); ++$i) {
            $this->items[$i]["products"] = array();
            foreach ($uDrawProducts->posts as $post) {     
                $block_product_id = get_post_meta($post->ID, '_udraw_block_template_id', true);                
                
                // Convert String ( old type ) to Array ( new type )
                if (gettype($block_product_id) == 'string') {
                    $block_product_id = explode("HuhWhatOkay", get_post_meta($post->ID, '_udraw_block_template_id', true));
                }                
                if (count($block_product_id) > 0) {
                    for ($j = 0; $j < count($block_product_id); $j++) {                        
                        if ($this->items[$i]["ProductID"] == intval($block_product_id[$j])) {
                            $product_array = array();
