...
class uDraw_Clipart_Table extends WP_List_Table {
    ...
    function column_image_name ($item) {
        if (is_user_logged_in()) {
            if (current_user_can('delete_udraw_clipart_upload')) {
                $actions = array(
                    'delete' => sprintf('<a href="?page=%s" onclick="javascript: removeClipart(\'%s\');" id="delete-me-link">%s</a>', $_REQUEST['page'], $item['access_key'], __('Delete', 'udraw'))
                );
    ...
    function prepare_items()
    {
        ...
        if (!isset($_GET['filter'])) {
            $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE image_name LIKE $search OR user_uploaded LIKE $search OR tags LIKE $search OR category LIKE $search ORDER BY $orderby $order", $per_page), ARRAY_A);
        } else {
            if ($filter != 'uncategorized') {
                $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE category = '". $filter ."' AND (image_name LIKE $search OR user_uploaded LIKE $search OR tags LIKE $search) ORDER BY $orderby $order", $per_page), ARRAY_A);
            } else {
                $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE category = '' OR category IS NULL AND (image_name LIKE $search OR user_uploaded LIKE $search OR tags LIKE $search) ORDER BY $orderby $order", $per_page), ARRAY_A);
            }
        }
