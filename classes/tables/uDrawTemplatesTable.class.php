...
class uDraw_Templates_Table extends WP_List_Table {
    ...
    {
        $uDraw = new uDraw();        
        return '<a href="?page=udraw_modify_template&template_id='. $item['id'] .'"><img src ="' . $item['preview'] . '?'. $uDraw->generateRandomString() .'" alt="Preview Template" style="max-height:50px;"></img></a>';
    }
    ...
    function prepare_items()
    {
        ...
        if (!isset($_GET['filter'])) {
            ...
            if ($category != 'uncategorized') {
                ...
                $this->items = $wpdb->get_results($wpdb->prepare("SELECT id, name, design, preview, pdf, create_date, create_user, modify_date, CONCAT(design_width,'\" x ', design_height,'\"') as size, design_pages, tags, category FROM $table_name WHERE (public_key IS NULL) AND ($catSQL) AND (name LIKE $search OR tags LIKE $search) ORDER BY $orderby $order", array()), ARRAY_A);
            } else {
                $this->items = $wpdb->get_results($wpdb->prepare("SELECT id, name, design, preview, pdf, create_date, create_user, modify_date, CONCAT(design_width,'\" x ', design_height,'\"') as size, design_pages, tags, category FROM $table_name WHERE public_key IS NULL AND (name LIKE $search OR tags LIKE $search) AND (category='' OR category IS NULL)  ORDER BY $orderby $order", array()), ARRAY_A);
            }
        }
